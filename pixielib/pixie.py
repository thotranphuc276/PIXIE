...
class PIXIE(object):
    ...
    def encode(self, data, threthold=True, keep_local=True, copy_and_paste=False, body_only=False):
        ...
        for key in data.keys():
            ...
            if key == 'head' or key == 'hand':
                ...
                part_dict = self.decompose_code(self.Regressor[part](feature[key][part]),
            ...
            if key == 'body':
                ...
                if 'head_image' not in data[key].keys() \
                    ...
                    # body only
                    body_dict = self.decompose_code(self.Regressor[part](feature[key][part]),
                    ...
                    for part_name in ['head', 'left_hand', 'right_hand']:
                        ...
                        cropped_image, cropped_joints_dict = self.part_from_body(data['body']['image_hd'], part_name,
                                                                                 points_dict)
                ...
                for part_name in ['head', 'left_hand', 'right_hand']:
                    ...
                    part_share_dict = self.decompose_code(self.Regressor[f'{part}_share'](f_part),
                                                          self.param_list_dict[f'{part}_share_list'])
    ...
    def convert_pose(self, param_dict, param_type):
        ...
        if param_type == 'head':
            ...
            param_dict['partbody_pose'] = self.smplx.body_pose.unsqueeze(0).expand(batch_size, -1, -1, -1)[:,
    ...
    def decode(self, param_dict, param_type):
        ...
        if param_type == 'head' or param_type == 'body':
            param_dict['body_pose'] = self.smplx.pose_abs2rel(param_dict['global_pose'], param_dict['body_pose'],
                                                              abs_joint='head')
        ...
        if self.cfg.model.check_pose:
            ...
            for pose_ind in [14]:  # head [15-1, 20-1, 21-1]:
                ...
                for i, max_angle in enumerate([20, 70, 10]):
                    ...
                    euler_pose_curr[euler_pose_curr != torch.clamp(euler_pose_curr, min=-max_angle * np.pi / 180,
        ...
        if 'tex' in param_dict.keys() and self.cfg.model.use_tex:
            ...
            texcode = torch.zeros([verts.shape[0], self.cfg.model.n_tex], device=self.device, dtype=torch.float32)
            albedo = self.flametex(texcode)
            prediction['albedo'] = albedo
        else:
            prediction['albedo'] = torch.zeros([verts.shape[0], 3, self.cfg.model.uv_size, self.cfg.model.uv_size],
